...
What it does:
SSHs into SW-ACCESS-01 and RT-BRANCH-01
Pushes the complete switch config (VLANs, access ports, trunk, port shutdown)
Pushes the complete router config (sub-interfaces, ACLs, SSH hardening)
Saves both configurations
...
Key Takeaways
VLANs isolate broadcast domains — HR traffic does not flood into IT unless explicitly routed.
802.1Q trunking allows a single physical link to carry multiple VLANs using 4-byte tags.
Router sub-interfaces (interface g0/0/0.10) create logical gateways for each VLAN without requiring separate physical cables.
Extended ACLs enforce security policies at the network edge — controlling which departments can communicate.
SSH replaces Telnet for encrypted remote management; RSA keys, local users, and banners are mandatory.
Unused ports should always be shutdown — physical security is network security.
Automation eliminates human error — Python/Netmiko and Ansible turn 30 minutes of CLI typing into 30 seconds of code execution.
Verification is non-negotiable — show vlan brief, show ip route, show ip access-lists, and ping are your best friends.
Documentation matters — Every troubleshooting step teaches something. Writing it down turns a lab into a portfolio piece.
File Structure
plain
Copy
